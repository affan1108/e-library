            ...
            <div class="portlet light portlet-fit portlet-datatable bordered">
                <div class="portlet-title">
                    <div class="caption">
                        ...
                        <span class="caption-subject font-dark sbold uppercase">User Level </span>
                        <a href="{{ route('stability.createUserLevel') }}" class="btn btn-xs btn-outline blue"><i class="fa fa-plus"></i> Tambah</a>
                    </div>
                    <div class="actions">
                    ...
                    <table class="table table-striped table-hover table-checkable dt-responsive" width="100%" id="example">
                        <thead>
                            ...
                            <tr role="row" class="filter">
                                ...
                                <td><select name="jabatan_id" class="select2" style="width:100%">
                                        <option value="">All</option>
                                        @foreach ($jabatans as $r)
                                            <option value="{{ $r->id }}">{{ $r->nama }}</option>
                                        @endforeach
                                    </select></td>
                                <td><select name="level_id" class="select2" style="width:100%">
                                        <option value="">All</option>
                                        @foreach ($levels as $r)
                                            <option value="{{ $r->id }}">{{ $r->nama }}</option>
                                        @endforeach
...
var TableDatatablesAjax = function () {
    ...
    grid.init({
        ...
        dataTable: {
            "ajax": {
                "url": "{{ route('stability.ajaxListUserLevel') }}", // ajax source
                "type": "POST",
                data: function (d) {
                    d._token        = "{{ csrf_token() }}";
                    ...
                    d.level_id           = $('.filter').find('[name=level_id]').val();
                }
            },
            columns: [
                ...
                { data: 'level_nama', name: 'm_levels.nama' },
...
function btn_delete(id) {
    var url = "{{ route('stability.destroyUserLevel', ':id') }}";
    url = url.replace(':id', id);
    swal({
        title: "Apakah Anda yakin?",
        text: "Level pada User ini akan dihapus!",
        type: "warning",
